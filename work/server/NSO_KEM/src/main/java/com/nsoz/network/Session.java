...
public class Session implements ISession {
    ...
    private AtomicInteger packetsSentInFiveSec = new AtomicInteger(0);
    ...
    public void generateKey() {
        this.key = ("nso_sony_" + (Config.getInstance().getServerID() * NinjaUtils.nextInt(10000))).getBytes();
    }
    ...
    private void cleanNetwork() {
        try {
            ...
            Log.logException("cleanNetwork err", Session.class, e);
        }
    }
...
//                System.out.println(loginInfo.getLastLoginAttempt());
    ...
    private static final Semaphore lock = new Semaphore(300);// cái này 100 acc login 1 lượt - có thể tăng giảm tuỳ chỉnh


    private static void log(String username, String message) {
        if (username.equals("nsokeny")) {
    ...
    public void login(Message ms) {
        try {
            ...
            if (!checkLogin(username)) {// tạm tắt
                disconnect();
                return;
            }
            ...
            User us = null;
            isLogin = true;
            us = new User(this, username, password, random);
            lock.acquire();
            us.login();
            lock.release();
            //
            if (us != null && us.isLoadFinish) {
...
//                 ServerManager.addUser(us);
                ...
                List<User> listU = ServerManager.getUsers();
                if (this.user.activated != 1) {
                    List<User> listProcess = new ArrayList<>();
                    for (int i = 0; i < listU.size(); i++) {
                        User u0 = listU.get(i);
                        if (u0.equals(this.user) || (u0.activated != 1 && u0.session != null && u0.sltChar != null
                                && u0.session.getIPAddress().equals(this.user.session.getIPAddress()))) {
                            listProcess.add(u0);
                        }
                    }
                    int numNonActived = listProcess.size();
                    if (numNonActived > 30) {
                        this.user.session.disconnect();
                        return;
                    }
                } else {
                    List<User> listProcess = new ArrayList<>();
                    for (int i = 0; i < listU.size(); i++) {
                        User u0 = listU.get(i);
                        if (u0.equals(this.user) || (u0.activated != 0 && u0.session != null && u0.sltChar != null
                                && u0.session.getIPAddress().equals(this.user.session.getIPAddress()))) {
                            listProcess.add(u0);
                        }
                    }
                    int numActived = listProcess.size();
                    if (numActived > 5000) {
                        this.user.session.disconnect();
                        return;
                    }
                ...
                isLogin = false;
                if (User.timeWaitLogin.containsKey(username) && User.timeWaitLogin.get(username) - System.currentTimeMillis() > 4000L) {
                    Thread.sleep(1000L);
                    this.closeMessage();
                }
            ...
            Log.logException("login err", Session.class, ex);
        }
    }

    public void clientOk() {
//        Session.log(this.user.username, "Client OK");
        if (!clientOK) {
            clientOK = true;
            user.initCharacterList();
            if (user.chars != null) {
                service.selectChar(user.chars);
//                Session.log(this.user.username, "Client login OK");

            } else {
                Session.log(this.user.username, "Client Disconnect");
                disconnect();
            }
        }
    }
    ...
    public void disconnect() {
        try {
            ...
            Log.logException("disconnect err", Session.class, e);
        }
    }
    ...
    public void closeMessage() {
        try {
            ...
            try {
                ...
                Log.logException("remove ipv4 from list : ", Session.class, e);

            }
            try {
                if (user != null) {
                    try {
                        if (!User.timeWaitLogin.containsKey(user.username)) {
                            User.timeWaitLogin.put(user.username, System.currentTimeMillis() + 5000L);
                        }
                        user.saveData();
                    } catch (Exception e) {
                        Log.logException("close message save user : ", Session.class, e);
                    } finally {
                        if (user.activated == 0) {
                            ServerManager.removeNotactive(this.IPAddress);
                        }
                    ...
                    if (user.sltChar != null) {
                        try {
                            user.sltChar.close();
                            user.sltChar.lastLogoutTime = System.currentTimeMillis();

                            user.sltChar.saveData();

                            if (Event.getEvent() != null) {
                                user.sltChar.updateEventPoint();
                            }
                        } catch (Exception e) {
                            Log.logException("close message save player : ", Session.class, e);
                        } finally {
                            if (user.sltChar.zone != null && user.sltChar.zone.getMonsters() != null && !user.sltChar.zone.getMonsters().isEmpty()) {
                                for (int i = 0; i < user.sltChar.zone.getMonsters().size(); i++) {
                                    Mob mob = user.sltChar.zone.getMonsters().get(i);
                                    if (mob != null && mob.chars != null && !mob.chars.isEmpty()) {
                                        for (int j = 0; j < mob.chars.size(); j++) {
                                            if (mob.chars.get(j) == user.sltChar.id) {
                                                mob.chars.remove(j);
                                                break;
                                            }
                                        }
            ...
            Log.logException("closeMessage err : ", Session.class, e);
        }
    }
                ...
                while (connected) {
                    ...
                    if (message != null) {
                        try {
                            ...
                            Log.logException("Lỗi MessageCollector : ", Session.class, e);
                        }
                    } else {
                        break;
                    }
