...
public class Battlefield extends Zone {
    ...
    public void mobDead(Mob mob, Char killer) {
        ...
        if (killer != null) {
// fix điểm ct muốn + cho tất car thì xóa  && killer.war.type != War.TYPE_ALL_LEVEL
            if (mob.levelBoss == 0 && killer.war != null ) {
                killer.addWarPoint((short) 1);
            }
            if (mob.levelBoss == 1 && killer.war != null ) {
                killer.addWarPoint((short) 5);
            }
            if (mob.levelBoss == 2 && killer.war != null) {
