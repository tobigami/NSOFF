...
public class Arena extends World {
    ...
    public void out(Char _char) {
        try {
            if (!isClosed) {
                // Check if teamOne is not null and not empty before accessing it
                ...
                // Check if teamTwo is not null and not empty before accessing it
            ...
            // Ensure the lock is acquired before modifying the lists
            lock.writeLock().lock();
            try {
                if (teamOne != null) {
            ...
            // Call removeMember only if _char is not null
            if (_char != null) {
                removeMember(_char);
                Zone z = zone;
                if (z != null) {
            ...
            Log.logException("Rời lôi đài lỗi: ", Arena.class, e);
        }
    }
    ...
    public void open() {
        ...
        if (leaderTeamOne.coin < moneyTeamOne) {
            service.serverMessage(leaderTeamOne.name + " Không đủ xu để bắt đầu");
            return;
        }
        if (leaderTeamTwo.coin < moneyTeamTwo) {
            service.serverMessage(leaderTeamTwo.name + " Không đủ xu để bắt đầu");
            return;
        }
        this.isOpened = true;
        zone.close();
        ...
        if (moneyTeamOne >= 1000000) {
            GlobalService.getInstance().chat("server",
    ...
    public void close() {
        try {
            ...
            Log.logException("Đóng lôi đài lỗi: ", Arena.class, e);
    ...
    public void update() {
        ...
        if (countDown <= 0) {  // fix bug lôi đài
            if (!finished) {
                if (isOpened) {
                    ...
                    service.serverMessage("Trận đấu đã bị hủy.");
                }
            }
            close();
            return;
