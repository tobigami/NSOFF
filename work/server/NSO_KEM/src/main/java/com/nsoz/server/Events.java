...
public class Events {
    ...
    public static void liXi(Char _char, String content) {
        ...
        if (NinjaUtils.nextBoolean()) {
            ...
            if (itemId == ItemName.BAT_BAO || itemId == ItemName.RUONG_BACH_NGAN || itemId == ItemName.RUONG_HUYEN_BI) {
                GlobalService.getInstance().chat("server", _char.name + " lì xì nhận được " + itm.template.name);
            }
    ...
    public static void matNaHo(Char p) {
        ...
        if (indexBanhTet == -1 || p.bag[indexBanhTet] == null || p.bag[indexBanhTet].getQuantity() < 1) {
            p.getService().npcChat(NpcName.TIEN_NU, "Bạn cần có đủ 20 chiếc bánh tét ");
            return;
        }
        if (p.getSlotNull() == 0) {
            p.getService().npcChat(NpcName.TIEN_NU, p.language.getString("BAG_FULL"));
    ...
    public static void doiCup(Char p, int type) {
        ...
        if (p.getEventPoint().getPoint(EventPoint.DIEM_TIEU_XAI) < point) {
            p.getService().npcChat(NpcName.TIEN_NU, "Bạn cần tối thiểu " + NinjaUtils.getCurrency(point) + " điểm thi đấu mới có thể đổi được chiếc cúp này.");
            return;
        }
    ...
    public static void doiLinhVat(Char p, int type) {
        int point = type == 1 ? 5000 : 20000;
        if (p.getEventPoint().getPoint(EventPoint.DIEM_TIEU_XAI) < point) {
            p.getService().npcChat(NpcName.TIEN_NU, "Bạn cần tối thiểu " + NinjaUtils.getCurrency(point) + " điểm sự kiện mới có thể đổi được vật này.");
            return;
        }
...
//        if (event == TET) {
//            Map map = MapManager.getInstance().find(4);
...
//                                MobTemplate template = MobManager.getInstance().find(231);
//                                Mob monster = new Mob(z.getMonsters().size(), (short) template.id, template.hp, template.level, (short) 1928, (short) 240, false, template.isBoss(), z);
//                                z.addMob(monster);
//                                mouseMob = monster;
//
...
//        }
    ...
    public void showLeaderBoard(Char _char, String format) {
        ...
        try {
            ...
            int n = 1;
            for (LeaderBoard leader : leaders) {
                text += n + ". " + leader.name + " " + String.format(format, leader.point) + "\n";
                n++;
            }
