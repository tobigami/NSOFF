...
public class GameData {
    ...
    public static final long DELAY_SAVE_DATA = ConstTime.MINUTE;
    public static final long DELAY_REFESH_RANK = ConstTime.MINUTE;
    ...
    public static final int[] GOLD_UP = {1, 2, 3, 4, 5, 10, 15, 20, 50, 100, 150, 200, 300, 400, 500, 600};
    public static final int[] MAX_PERCENT = {80, 75, 70, 65, 60, 55, 50, 45, 40, 35, 30, 25, 20, 15, 10, 2};
    public static final int[] MAX_PERCENT_PUSH = {80, 75, 70, 65, 60, 55, 50, 45, 40, 35, 30, 25, 20, 15, 10, 5};
...
//        }
    ...
    private void loadSkillOption() {
        try (Connection conn = DbManager.getConnection()) {
            ...
            PreparedStatement stmt = conn.prepareStatement("SELECT * FROM `skill_option`;");
            ...
            Log.logException("Load skill option:", GameData.class,ex);
...
//
//                    final org.json.simple.JSONArray Skilltemplate = (org.json.simple.JSONArray) JSONValue.parse(rs2.getString("SkillTemplates"));
//                    for (final Object template : Skilltemplate) {
//                        final org.json.simple.JSONObject job2 = (org.json.simple.JSONObject) template;
//                        final Skill skill = new Skill();
//                        skill.id = Short.parseShort(job2.get((Object) "skillId").toString());
//                        skill.point = Byte.parseByte(job2.get((Object) "point").toString());
//                        skill.level = Byte.parseByte(job2.get((Object) "level").toString());
//                        skill.manaUse = Short.parseShort(job2.get((Object) "manaUse").toString());
//                        skill.coolDown = Integer.parseInt(job2.get((Object) "coolDown").toString());
//                        skill.dx = Short.parseShort(job2.get((Object) "dx").toString());
//                        skill.dy = Short.parseShort(job2.get((Object) "dy").toString());
//                        skill.maxFight = Byte.parseByte(job2.get((Object) "maxFight").toString());
//                        skill.template = skillTemplate;
//                        final org.json.simple.JSONArray Option2 = (org.json.simple.JSONArray) JSONValue.parse(job2.get((Object) "options").toString());
//                        skill.options = new SkillOption[Option2.size()];
//                        for (int i = 0; i < Option2.size(); i++) {
//                            final org.json.simple.JSONObject job3 = (org.json.simple.JSONObject) Option2.get(i);
//                            final SkillOption skillOption = new SkillOption(Integer.parseInt(job3.get((Object) "id").toString()), Integer.parseInt(job3.get((Object) "param").toString()));
...
////                        skillTemplate.addSkill(skill);
////
////                        System.out.println("skillTemplate: " + skillTemplate.id);
////                    }
////                    rs3.close();
////                    ps3.close();
...
//    }
    private void loadClass() {
        ...
        try {
            ...
            try {
                while (rs.next()) {
                    ...
                    try {
                        ...
                        Log.logException("Lỗi load clazz data(1) : ", GameData.class, e);
            ...
            Log.logException("Lỗi load clazz data(2) : ", GameData.class, ex);
        } finally {
            DbManager.closeConnection(conn);
        }
    }
    ...
    public void writeDataMobNew(DataOutputStream ds, MobTemplate mob) throws IOException {
        ...
        for (ImageInfo image : mob.imgInfo) {
            ...
            ds.writeByte(image.x);
            ds.writeByte(image.y);
    ...
    public static void saveFile(String url, byte[] data) {
        try {
            ...
            Log.logException("Lỗi saveFile : ", GameData.class, e);
