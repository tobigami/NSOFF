...
public class StoreManager {
    ...
    public void init() {
        ...
        try {
            ...
            PreparedStatement ps = conn.prepareStatement(SQLStatement.GET_ALL_STORE);
            ...
            while (rs.next()) {
                ...
                if (id == TYPE_CLAN) {
                    ...
                    // MaxStore cư xử y hệt Store trừ khi người mua bật công tắc "hàng max", nên
                    // thay ở đây là an toàn cho mọi cửa hàng.
                    add(new MaxStore(id, name));
                }
            }
            rs.close();
            ps.close();
        } catch (SQLException ex) {
            Log.logException("Load init store:", Store.class,ex);
        } finally {
    ...
    public void addItem(byte storeType, ItemStore item) {
    ...
    public ItemStore getItemWithId(int itemId) { // shop 16
        ...
        for (int type : types) {
            ...
            if (store != null) {
                for (ItemStore t : store.getItems()) {
                    if (t.getTemplate().id == itemId) {
                        return t; // Trả về ngay khi tìm thấy
                    }
                }
            }
        }
        return null; // Không tìm thấy item
    }
    ...
    public List<ItemStore> getListEquipmentWithLevelRangeNotWeapon(int start, int end) {
        ArrayList<ItemStore> items = new ArrayList<>();
        int[] types = {TYPE_NECKLACE, TYPE_RING, TYPE_PEARL, TYPE_SPELL, TYPE_MEN_HAT, TYPE_MEN_SHIRT, TYPE_MEN_GLOVES, TYPE_MEN_PANT, TYPE_MEN_SHOES,
                TYPE_WOMEN_HAT, TYPE_WOMEN_SHIRT, TYPE_WOMEN_GLOVES, TYPE_WOMEN_PANT, TYPE_WOMEN_SHOES};
        for (int type : types) {
            Store store = find((byte) type);
            if (store != null) {
    ...
    public List<ItemStore> getListWeaponRandom(int level) {
        ArrayList<ItemStore> items = new ArrayList<>();
        Store store = find((byte) 2);
        if (store != null) {
            store.stream().forEach((t) -> {
                ItemTemplate template = t.getTemplate();
