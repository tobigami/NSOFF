...
public class ThanThu {
    ...
    public static final byte HOA_LONG = 2;
    ...
    public void levelUp() {
        ...
        if (this.level <= 100) {
            ...
            // fix  lv thần thú khi lv up

            for (ItemOption o : options) {
                int multi;
                if (o.optionTemplate.id == ST_QUAI_ID) {
                    multi = type == HOA_LONG ? NinjaUtils.nextInt(8, 13) : NinjaUtils.nextInt(5, 10);
                } else {
                    multi = type == HOA_LONG ? NinjaUtils.nextInt(2, 7) : NinjaUtils.nextInt(1, 5);
                }

                int add = stars * multi * levelPer10;
                o.param += add;
                setAttribute(o);
            }
        }
    ...
    public String getName() {
        if (eggHatchingTime > 0) {
            if (type == HAI_MA) {
                return "Trứng Hải Mã";
            } else if (type == HOA_LONG) {
                return "Trứng Hỏa Long";
            } else {
