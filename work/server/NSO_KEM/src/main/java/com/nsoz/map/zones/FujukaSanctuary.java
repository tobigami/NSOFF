...
public class FujukaSanctuary extends AreaWithCountdownTime {
    ...
    static final String[] recoveryMessages = {"Mỗi lần bạn chết, ta lại cảm thấy cơ thể tràn đầy năng lượng", "Wowww, nguồn sức mạnh này", "Có vẻ như bạn là một phiên bản thất bại"};
    static final String[] defiantMessages = {"Bạn cũng chỉ là một trong số chúng ta, nhưng là phiên bản yếu kém hơn", "Hãy đón lấy những đòn tấn công này đi", "Sức mạnh của bạn là một thứ gì đó rẻ mạt"};
...
//        for (Char p : chars) {
//            if (!p.isNhanBan && !(p instanceof Bot)) {
//                p.addClanPoint(20);
//                Item item = ItemFactory.getInstance().newItem(ItemName.RUONG_HAC_AM);
//                item.quantity = getChestQuantity();
//                item.isLock = true;
//                p.addItemToBag(item);
//
//                item = ItemFactory.getInstance().newItem(ItemName.KHOA_HAC_AM);
//                item.quantity = NinjaUtils.nextInt(1, getChestQuantity());
//                item.isLock = false;
//                p.addItemToBag(item);
//            }
//        }
        this.finished = true;
        setTimeMap(60);
        getService().serverMessage("Đã tiêu diệt hết dị bản, chúc mừng bạn");
    }
