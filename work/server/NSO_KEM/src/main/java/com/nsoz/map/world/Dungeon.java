...
public class Dungeon extends World {
        ...
        if (level == 5) {
            ...
            for (Zone zone : zones) {
                ...
                if (numberLiving > 0) {
                    ...
                    if (!isBossLive) {
                        zone.killAllMonsters(true);

                        addPointPB(numberLiving); // fix tích lũy hd9x
    ...
    public void finish() {
        ...
        members.forEach(_char -> {
            try {
                ...
                Log.logException("Kết thúc hang động lỗi ", Dungeon.class, e);
            }
        });
        finished = true;
    ...
    public void addPointPB(int point) {
        ...
        for (Char _char : members) {
            try {
                ...
                Log.logException("Add điểm hang động lỗi", Dungeon.class, e);
    ...
    public void addExp(Char _c, long exp) {
        ...
        for (Char _char : members) {
            try {
                if (_char != _c) {
                    _char.addExp(exp); // exp hang động
                }
            } catch (Exception e) {
                Log.logException("Add exp hang động lỗi", Dungeon.class, e);
    ...
    public void close() {
        ...
        for (Char _char : members) {
            try {
                ...
                Log.logException("Đóng hang động lỗi", Dungeon.class, e);
