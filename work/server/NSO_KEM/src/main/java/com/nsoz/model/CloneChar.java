...
public class CloneChar extends Char {
        ...
        try (Connection conn = DbManager.getConnection()) {
            ...
            try {
                if (res.next()) {
                    ...
                    ArrayList<Item> obj2 = human.getMaskBox();
                    if (obj2 != null) {
                        this.setMaskBox(obj2);
                        for (int i = 0; i < obj2.size(); i++) {

                            Item item = obj2.get(i);
                            if (item.id == this.getMaskId()) {
                                this.setMask(item);
                            }
                        }
                    }
                    ArrayList<Item> obj3 = human.getCollectionBox();
                    if (obj3 != null) {
                        this.setCollectionBox(obj3);
                    }
                    this.exp = parse.getLong("exp");
                    this.level = NinjaUtils.getLevel(this.exp);
                    // Phân thân được phép khác giới tính với chủ thể. Hai cột `gender` và `head`
                    // của clone_char để -1 thì theo chủ y như cũ, đặt số thật thì phân thân dùng
                    // số đó. Phải đi kèm nhau: đổi giới mà giữ tóc của chủ thì ra mặt sai giới.
                    byte gioiRieng = res.getByte("gender");
                    short dauRieng = res.getShort("head");
                    this.head = this.original_head = dauRieng >= 0 ? dauRieng : human.original_head;
                    this.clan = null;
                    this.user = human.user;
                    this.gender = gioiRieng >= 0 ? gioiRieng : human.gender;
                    ...
                    if (jso != null) {
                        ...
                        for (int i = 0; i < size; i++) {
                            ...
                            if ((equipe.hasExpire() && System.currentTimeMillis() > equipe.expire) || equipe.isRemoveItem() || isInList(equipe.template.id)) {
                                continue;
                            }
                    ...
                    DbManager.executeUpdate("INSERT INTO `clone_char`(`id`) VALUES (?);", this.id);
                    load();
                }
            } catch (Exception e) {
                Log.logException("Lỗi load data clone player (1)", CloneChar.class, e);
            ...
            Log.logException("Lỗi load data clone player (2)", CloneChar.class, e);
        }
        return true;
    }
    ...
    public void startDie() {
        if (!isNhanBan) {
            this.isDead = true;
            this.timeCountDown = human.timeCountDown = 0;
            human.switchToMe();
        }
    }
        ...
        if (isLoadFinish() && !saving) {
            ...
            try (Connection conn = DbManager.getConnection()) {
                ...
                try {
                    ...
                    Log.logException("Lỗi save clone player to mongoDB", CloneChar.class, e);
                ...
                Log.logException("Lỗi save data clone player (1)", CloneChar.class, e);
    ...
    public void switchToMe() {
        ...
        System.out.println("[doi-than] " + human.name + " chu than -> phan than, map " + human.mapId
                + " (" + human.x + "," + human.y + ") con " + human.timeCountDown + "s");
        this.box = human.box;
        this.bag = human.bag;
