...
import com.nsoz.map.world.WorldService;
...
public class ThienDiaData {
    ...
    public void updateData() {
        try (Connection conn = DbManager.getConnection()) {
            synchronized (rankings) {
                PreparedStatement ps = conn.prepareStatement("UPDATE `ranking_list` SET `match_id`=?,`rank_at`=? WHERE `id` = ?");
               try{
                   for (Ranking ranking : rankings) {
                       ps.setInt(1, getId());
             ...
             Log.logException("update Thien dia bang  err: " , ThienDiaData.class, ex);
        }
    }
    ...
    public void load() {
        try (Connection conn = DbManager.getConnection();) {
            ...
            try {
             ...
             Log.logException("load TDB err: " , ThienDiaData.class, ex);
        }
    }
