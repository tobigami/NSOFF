...
public class TalentShow extends Zone {
    ...
    public void startTlShow() {
        ...
        setTimeMap(300); /// thời gian 2 bên pk
    ...
    public void finish() {
        finished = true;
        setTimeMap(10); /// thời gian sau khi đánh xong
    }
    ...
    public void removePlayer(Char _char) {
        ...
        try {
            ...
            Log.logException("Ninja Tài năng lỗi removePlayer: ", TalentShow.class, e);
        } finally {
            lock.writeLock().unlock();
        }
    }
    ...
    public void addTalent(Char p, String name) { /// thêm ninja tài năng
        p.menus.clear();
    ...
    public void showMenu(Char _charz) {  /// thêm ninja tài năng
        ArrayList<Menu> menus = _charz.menus;
        menus.clear();
        if (_charz.user.isTien()) {
        ...
        synchronized (players) {
            for (Char _char2 : players) {
                try {
                    ...
                    Log.logException("Ninja Tài năng lỗi loadMap: ", TalentShow.class, e);
    ...
    class Player {
        ...
        public void join() {
            if (isAttendance()) {
                ...
                player.setNonCombatState(10); /// time lên trên chờ
            }
        }
        ...
        public void out() {
            if (isAttendance()) {
                ...
                player.getService().resetPoint();
                getService().teleport(player);
                if (player.clone != null && player.clone.isNhanBan && !player.clone.isDead) {  /// fix phân thân không xuống khi xong trận
