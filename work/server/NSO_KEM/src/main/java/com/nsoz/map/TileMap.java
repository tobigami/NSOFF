...
public class TileMap {
    ...
    public static final byte MAP_GTC = 5;
    ...
    public boolean isNotChangeZone() {
        return isDungeo() || isDungeoClan() || isLoiDai() || isFujukaSanctuary() || isCandyBattlefield() || id == MapName.HANG_NYMOZ || isMapVithu() || isThatThuAi();
    }

    public boolean isWorld() {
        return isDungeo() || isDungeoClan() || isThatThuAi() || isCandyBattlefield();
    }

    public boolean isNotBlack() {  /// t7 đen tối
        return isDungeo() || isDungeoClan() || isThatThuAi() || isCandyBattlefield() || isLoiDai() || isMapVithu() || isFujukaSanctuary() || isDauTruong();
    }

    public boolean isNotRivival() {
        return isLoiDai() || isDauTruong();
    }

    public boolean isNotPk() {
        return isLoiDai() || isChienTruong() || isDauTruong() || isFujukaSanctuary() || isCandyBattlefield() || isMapVithu();
    }

    public boolean isNotAllowPkPoint() { /// dev fix 13/8
        return isLangCo() || isLangTruyenThuyet() || isLangDiaNguc() ; // lang dia nguc
    }
    ...
    public boolean isLangDiaNguc() { // lang dia nguc
        return id == MapName.LANG_DIA_NGUC || id == MapName.QUY_NGUC;
    }
    public boolean isMapEvent() {
        return id == MapName.DONG_HACHI || id == MapName.KY_TUC_XA_HARUNA || id == MapName.SONG_BANG_YAMATO;
    }
    ...
    public boolean isLoiDai() { /// dev fix bug lôi đài 13/8
        return id == MapName.KHU_BAO_DANH || id == MapName.LOI_DAI || id == MapName.LOI_DAI_2 || id == MapName.LOI_DAI_3 || id == MapName.LOI_DAI_4;
    }
    ...
    public boolean isNotSave() {
        return isDungeo() || isDungeoClan() || isChienTruong() || isDauTruong() || isLoiDai() || isFujukaSanctuary() || isVDMQ()
    ...
    public boolean isDungeo() {
        return type == MAP_PB && !isMapVithu();
    }

    public boolean isDungeo9x() {
        return id == MapName.TAM_NHAN_QUAN_1 || id == MapName.TAM_NHAN_QUAN_2 || id == MapName.TAM_NHAN_QUAN_3;
    }
    ...
    public boolean isNotSummon() {
        return isDauTruong() || isChienTruong() || isDungeo() || isDungeoClan() || isLoiDai() || isCandyBattlefield();
    }

    public boolean isDauTruong() {
        return id == MapName.DAU_TRUONG; // MAP_DAUTRUONG
    }

    public boolean isChienTruong() {
        if (this.id == 99 || this.id == 100 || this.id == 101 || this.id == 102 || this.id == 103 || this.id == 118 || this.id == 119) {
            return true;
        }
        return false;
    }
    ...
    public boolean isCancu() {
        if (this.id == 98 || this.id == 104) {
            return true;
        }
        return false;
    }
    ...
    public boolean isGTC() {
        if (this.id == 117 || this.id == 120 || this.id == 121 || this.id == 122 || this.id == 123 || this.id == 124 || this.id == 98 || this.id == 104) {
            return true;
        }
        return false;
    }

    public boolean isMapVithu() {
        if (this.id >= 169 && this.id <= 175) {
            return true;
        }
        return false;
    }
    ...
    public void loadMap() {
        ...
        try {
            ...
            Log.logException("loadMap lỗi", TileMap.class, ex);
        }
    }
    ...
    public void loadMapFromResource() {
        try {
            ...
            Log.logException("loadMapFromResource lỗi", TileMap.class, ex);
