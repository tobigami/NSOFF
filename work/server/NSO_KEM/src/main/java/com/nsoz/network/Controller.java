...
public class Controller implements IMessageHandler {
    ...
    private static final int MAX_REQUESTS_GET_DATA = 20;
    private final java.util.Map<Byte, Integer> commandCount = new HashMap<>();

    public Controller(Session client) {
        this.client = client;
        commandCount.put(CMD.UPDATE_DATA, 0);
        commandCount.put(CMD.UPDATE_MAP, 0);
        commandCount.put(CMD.UPDATE_SKILL, 0);
        commandCount.put(CMD.UPDATE_ITEM, 0);
    }
    ...
    public void onMessage(Message mss) {
        // Ghi tên vào đầu mỗi gói tin: từ đây tới hết lượt xử lý, mọi dòng log đều biết của ai.
        // Ưu tiên tên nhân vật; chưa chọn nhân vật thì lấy tên tài khoản.
        Log.datAi(_char != null && _char.name != null ? _char.name
                : (user != null ? user.username : null));



        int count = client.getPacketsSentInFiveSec().incrementAndGet();
        int maxPacketAllowed = 1000;
        int packetFloodThreshold = 5000;
        int spamCheck = 1;

        if (count > maxPacketAllowed) {
            long time = System.currentTimeMillis() - client.getLastPacketSentAt();
            if (time < packetFloodThreshold) {
                SpammerManager.getInstance().lock(client.getIPAddress());
                client.getPacketsSentInFiveSec().set(0);
                client.setLastPacketSentAt(System.currentTimeMillis());
                client.closeMessage();
                return;
            } else {
                client.getPacketsSentInFiveSec().set(0);
            ...
            try {
                ...
                switch (command) {
                    ...
                    case CMD.GET_EFFECT:
                        ...
                        // Ghi vết MỌI yêu cầu hiệu ứng của máy khách.
                        //
                        // Trước đây chỉ ghi ở nhánh gửi dữ liệu (type 2), nên khi máy khách im
                        // lặng thì không phân biệt được "nó không hỏi" với "nó hỏi kiểu khác".
                        // Đây là mảnh cuối để chốt xem lỗi nằm ở máy chủ hay máy khách.
                        System.out.println("[xin-hieu-ung] "
                                + (_char == null ? "?" : _char.name) + " type=" + type);
                        if (type == 1) {
                ...
                Log.logException("Lỗi nhận message từ client: ", Controller.class, e);
    ...
    public void newMessage(Message mss) {
        if (mss != null) {
            try {
                ...
                Log.logException("Lỗi nhận newMessage từ client: ", Controller.class, e);
    ...
    public void messageNotLogin(Message mss) {
        if (mss != null) {
            try {
                ...
                if (command == CMD.UPDATE_DATA || command == CMD.UPDATE_MAP || command == CMD.UPDATE_SKILL || command == CMD.UPDATE_ITEM) {
                    commandCount.put(command, commandCount.getOrDefault(command, 0) + 1);
                    if (commandCount.get(command) > MAX_REQUESTS_GET_DATA) {
                        SpammerManager.getInstance().lock(client.getIPAddress());
                        client.closeMessage();
                        return;
                    }
                ...
                Log.logException("Lỗi nhận messageNotLogin từ client: ", Controller.class, e);
...
//                System.out.println("command: "+command);
                ...
                if (command == CMD.UPDATE_SKILL) { // fix phá
                    commandCount.put(command, commandCount.getOrDefault(command, 0) + 1);
                    if (commandCount.get(command) > MAX_REQUESTS_GET_DATA) {
                        SpammerManager.getInstance().lock(client.IPAddress);
                        client.closeMessage();
                        return;
                    }
                }
                ...
                switch (command) {
                    ...
                    case CMD.MOI_GTC: // mời 1 member  vào gtc
                        _char.inviteMemberGTC(mss);
                        break;
                    case CMD.MOI_TATCA_GTC: // mời all member  vào gtc

                        break;

                    case CMD.GO_GTCHIEN: //  vào gtc
                        _char.memberAppectGTC();
                        break;
...
//                    case CMD.ME_ACTIVE:
//                        _char.updateProtect(mss);
//                        break;

                    case CMD.ME_UPDATE_ACTIVE:
                        _char.updatePassProtect(mss);
                        break;

                    case CMD.ME_OPEN_LOCK:
                        _char.openProtect(mss);
                        break;
                    case CMD.ME_CLEAR_LOCK:
                        _char.clearProtect(mss);
                        break;
                    default:
                ...
                Log.logException("Lỗi nhận messageNotMap từ client: ", Controller.class, e);
    ...
    public void messageSubCommand(Message mss) {
        if (mss != null) {
            try {
                ...
                Log.logException("Lỗi nhận messageSubCommand từ client: ", Controller.class, e);
    ...
    public void onConnectOK() {
        System.out.printf("Client %d: Kết nối thành công!%n", client.id);
