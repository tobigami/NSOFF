        ...
        if (!listExpired.isEmpty()) {
            ...
            for (Effect eff : listExpired) {
                ...
                if (player.zone != null) {
                    ...
                    if (!player.isHuman) {
                        if (!player.isDead && player.zone.tilemap.isVDMQ()) {
                            byte id = eff.template.id;
                            if (id == 34) {
                                player.setXY((short) 122, (short) 144);
                                player.changeMap(MapName.LANG_TONE);
                            }
                        }
    ...
    public byte[] getData() {
        ...
        try {
            ...
            return ms.getData();
             ...
             Log.logException("Get data effManager err",EffectManager.class, ex);
        } finally {
            ms.cleanup();
        }
        return new byte[]{0};
    ...
    public JSONArray toJSONArray() {
        ...
        synchronized (list) {
            for (Effect eff : list) {
                ...
                if (type == 0 || type == 18 || type == 25 || type == 26 || type == 28 || type == 44) {
