...
/**
 * Rút gọn máu quái ở dòng thông tin trên đầu quái.
 *
 ...
 *
 * Usage: ShortMobHp <in.jar> <out.jar>
 */
public final class ShortMobHp {
    ...
    private static final String HOOK = "SoNgan", HOOK_METHOD = "themI";
    private static final String DESC = "(Ljava/lang/StringBuffer;I)Ljava/lang/StringBuffer;";

    public static void main(String[] args) throws Exception {
        if (args.length < 2) {
            System.out.println("usage: ShortMobHp <in.jar> <out.jar>");
            return;
        }
        ...
        System.out.println(tong > 0
                ? "  rút gọn máu quái ở dòng thông tin: " + tong + " chỗ"
                : "!! không thấy dòng thông tin quái -- chưa vá");
    }
    ...
    private static byte[][] patch(byte[] d) throws Exception {
        ...
        for (int i = 1; i < count; i++) {
            ...
            switch (tag) {
                ...
                case 16: case 19: case 20: copy(in, pool, 2); break;
        ...
        for (int i = 1; i < count; i++) {
            ...
            if ("java/lang/StringBuffer".equals(cls) && "append".equals(nm)
                    && "(I)Ljava/lang/StringBuffer;".equals(de)) {
                append = i;
            }

        }
        for (int i = 1; i < count; i++) {
            // Hằng thật là "Id: " kèm dấu cách cuối; javap cắt mất khi in chú thích nên dễ so nhầm.
            if (utf8[i] != null && utf8[i].startsWith("Id:")) {
                for (int k = 1; k < count; k++) {
                    if (strTarget[k] == i) {
                        moc = k;                          // hằng String trỏ vào nó
                        break;
                    }
                }
            }
        ...
        for (int i = 0; i + 2 < rest.length; i++) {
            // ldc_w <hằng "Id:"> -- mốc mở đầu dòng thông tin quái
            if ((rest[i] & 0xFF) != 0x13
                    || (((rest[i + 1] & 0xFF) << 8) | (rest[i + 2] & 0xFF)) != moc) {
                continue;
            }
            int doi = 0;
            for (int k = i + 3; k < i + 90 && k + 2 < rest.length && doi < 2; k++) {
                if ((rest[k] & 0xFF) != INVOKEVIRTUAL) {
                    continue;
                }
                if ((((rest[k + 1] & 0xFF) << 8) | (rest[k + 2] & 0xFF)) != append) {
                    continue;
                }
                rest[k] = (byte) INVOKESTATIC;
                rest[k + 1] = (byte) (moi >> 8);
                rest[k + 2] = (byte) moi;
                doi++;
                n++;
